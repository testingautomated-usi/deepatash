...
dnn_model_MLP = keras.models.Sequential([   
    keras.Input(shape=(24,)),     
    keras.layers.Dense(24, activation='relu'),       
    keras.layers.Dense(4,activation='relu'),
    keras.layers.Dense(1)
])

dnn_model_StdSA =keras.models.Sequential([        
    keras.Input(shape=(24,)),     
    keras.layers.Dense(24, activation='relu'),        
    keras.layers.Dense(4,activation='relu'),
    keras.layers.Dense(1)
...
dnn_model_behaviour = keras.models.Sequential([        
    keras.Input(shape=(24,)),     
...
def train_dnns(X, Y1, Y2, Y3):
    log.info("dnns are trained")
    tf.keras.backend.clear_session()
    callback = tf.keras.callbacks.EarlyStopping(monitor='loss', patience=5)

    _optimizer = Adam(lr=0.01)

    dnn_model_MLP.compile(optimizer=_optimizer, loss="mse", )
    dnn_model_MLP.fit(np.array(X), np.array(Y1), epochs=200, batch_size=32, verbose=1, validation_split=0.1, callbacks=[callback])

    dnn_model_StdSA.compile(optimizer=_optimizer, loss="mse")
    dnn_model_StdSA.fit(np.array(X), np.array(Y2), epochs=200, batch_size=32, verbose=1, validation_split=0.1, callbacks=[callback])

    dnn_model_behaviour.compile(optimizer=_optimizer, loss="mse")
    dnn_model_behaviour.fit(np.array(X), np.array(Y3), epochs=200, batch_size=32, verbose=1, validation_split=0.1, callbacks=[callback])


def evaluate_dnns(_X, y1_test, y2_test, y3_test):
    score1 = dnn_model_MLP.evaluate(np.array(_X), np.array(y1_test))
    score2 = dnn_model_StdSA.evaluate(np.array(_X), np.array(y2_test))
    score3 = dnn_model_behaviour.evaluate(np.array(_X), np.array(y3_test))

    log.info(f"acc dnn MLP: {score1}")
    log.info(f"acc dnn StdSA: {score2}")
    log.info(f"acc dnn behaviour: {score3}")
...
def evaluate_individual(individual, features, goal, archive, mock):
    ...
    if mock == False:
        ...
        evaluator.evaluate_mock(individual, dnn_model_MLP, dnn_model_StdSA, dnn_model_behaviour)
...
def main(name):
    ...
    if len(archive.archive) > 0:
        ...
        evaluate_dnns(x1, y1_test, y2_test, y3_test)
